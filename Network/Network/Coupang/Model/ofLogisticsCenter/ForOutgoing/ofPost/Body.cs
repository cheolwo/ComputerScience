...
namespace Coupang.Model.ofLogisticsCenter.ForOutging.ofPost
{
    public class Body
    {
...
/*
   ...
   400 (요청변수확인)	배송비는 0원, 1000원 이상 20000원 이하만 가능합니다.	jeju, notJeju 값을 올바른 범위로 입력했는지 확인합니다. 
   도서산간배송비를 0 또는 최소 1,000원 이상 최대 20000원 이하로 입력합니다.
   ...
   400 (요청변수확인)	The format of phone number is invalid, min length is 11 and max length is 14 include the character '-'	
   companyContactNumber 값을 올바른 형식으로 입력했는지 확인합니다. '-'를 포함하여 최소 11자 이상 최대 14자 이하로 입력합니다.
   400 (요청변수확인)	The format of postal code is invalid, it can only be number, min length is 5 and max length is 6	
   returnZipCode 값을 올바르게 입력했는지 확인합니다. 숫자로만 최소 5, 최대 6자 이하로 입력합니다. 
    */